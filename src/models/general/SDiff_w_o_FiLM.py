...
class SDiff_w_o_FiLM(BaseModel):
    reader = 'BaseReader'
    runner = 'SDiffRunner'  # 可复用相同 Runner
    ...
    def __init__(self, args, corpus):
        ...
        # 关键区别：使用无 FiLM 的 Denoiser
        self.denoiser = SDiffDenoiser_woFiLM(self.emb_size, self.K)
...
# ==================== 无 FiLM 的 Denoiser：纯拼接 ====================
class SDiffDenoiser_woFiLM(nn.Module):
    def __init__(self, emb_size, K):
        super().__init__()
        # 输入：[vt; c_spectral; time_feat] → total dim = K + K + emb_size
        self.mlp = nn.Sequential(
            nn.Linear(K + K + emb_size, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, K)
    ...
    def forward(self, vt, c_spectral, time_feat):
        # 拼接所有信息
        combined = torch.cat([vt, c_spectral, time_feat], dim=-1)  # [B, K + K + emb_size]
