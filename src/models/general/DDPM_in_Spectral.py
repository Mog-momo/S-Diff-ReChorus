...
class DDPM_in_Spectral(BaseModel):
    reader = 'BaseReader'
    runner = 'SDiffRunner'
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--K_eig', type=int, default=200)
    ...
    def forward(self, feed_dict):
        ...
        if self.training:
            ...
            t_k = torch.randint(1, self.T + 1, (B,), device=self.device)  # [B]
            t_norm = t_k.float() / self.T  # t ∈ (0, 1]

            # >>> 各向同性：不使用 eigenvals，所有维度共享相同衰减 <<<
            lam_t_scalar = torch.exp(-t_norm)  # [B]
            lam_t = lam_t_scalar.unsqueeze(1).expand(-1, self.K)  # [B, K]
            ...
            for step in reversed(range(1, self.T + 1)):  # step = T, ..., 1
                t_val = torch.full((B,), step / self.T, device=self.device)  # [B]

                # >>> 各向同性：统一衰减 <<<
                lam_t_scalar = torch.exp(-t_val)  # [B]
                lam_t = lam_t_scalar.unsqueeze(1).expand(-1, self.K)  # [B, K]
                alpha_t = lam_t
                sigma_t = torch.sqrt(1 - alpha_t ** 2).clamp(max=self.sigma_max)
                ...
                if step > 1:
                    ...
                    lam_prev_scalar = torch.exp(-t_prev)
                    lam_prev = lam_prev_scalar.unsqueeze(1).expand(-1, self.K)
                    alpha_prev = lam_prev
            ...
            full_pred = v_curr @ self.U.T  # [B, item_num]
            full_pred = full_pred * (1 - x0)  # 屏蔽历史交互
...
class SDiffDenoiser(nn.Module):
    def __init__(self, emb_size, K):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(K * 2, 512), nn.ReLU(),
            nn.Linear(512, 256), nn.ReLU(),
            nn.Linear(256, K)
        )

    def forward(self, vt, c_spectral, time_feat):
        gamma = self.film_gamma(time_feat)
        beta = self.film_beta(time_feat)
        c_modulated = gamma * c_spectral + beta
        combined = torch.cat([vt, c_modulated], dim=-1)
